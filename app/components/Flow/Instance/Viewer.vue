                    ...
                    <div class="node-header">
                      ...
                      <div class="element-info">
                        ...
                        <div class="element-meta">
                          ...
                          <span v-if="data.status && data.type === 'action'" class="status-tag" :class="'status-' + data.status">
                            <svg v-if="data.status === 'completed'" width="12" height="12" fill="currentColor" viewBox="0 0 24 24">
                          ...
                          <span v-if="data.completedAt && data.type === 'action'" class="completion-date">
                            <svg width="12" height="12" fill="currentColor" viewBox="0 0 24 24">
                              <path d="M19 3h-1V1h-2v2H8V1H6v2H5c-1.11 0-1.99.9-1.99 2L3 19c0 1.1.89 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm0 16H5V8h14v11zM7 10h5v5H7z"/>
                            </svg>
                            Completed: {{ formatDate(new Date(data.completedAt).toISOString()) }}
                          </span>
          ...
          <aside class="template-sidebar">
            <div class="sidebar-content">
              <div class="info-item">
                ...
                <div class="info-value">{{ flow?.name || 'Unnamed Flow' }}</div>
              </div>
              <div class="info-item">
                <label>Description</label>
                <div class="info-value description-text">{{ flow?.description || 'No description' }}</div>
              </div>
              <div class="info-item">
                ...
                <div class="info-value">
                  <div class="progress-info">
                    <div class="progress-bar">
                      <div class="progress-fill" :style="{ width: `${getFlowProgress()}%` }"></div>
                    </div>
                    <span class="progress-text">{{ getFlowProgress() }}%</span>
                  </div>
                </div>
              </div>
              <div class="info-item">
                ...
                <div class="info-value duration-range">
                  <span class="duration-number">{{ flow?.expectedEndDate ? formatDate(flow.expectedEndDate) : 'No date set' }}</span>
                </div>
              </div>
              <div class="info-item">
                <label>{{ flow?.completedAt ? 'Completed' : 'Started' }}</label>
                <div class="info-value">
                  {{ flow?.completedAt ? formatDate(flow.completedAt) : (flow?.startedAt ? formatDate(flow.startedAt) : 'Not started') }}
                </div>
              </div>
            </div>
          </aside>
...
const downloadCsv = () => {
  ...
  if (props.flow.relations) {
    props.flow.relations.forEach(rel => {
  ...
  let csv = 'id,name,description,type,status,owner,consultedUsers,completedAt,expectedEndedAt,predecessorIds,predecessorTypes,successorIds,successorTypes\n'
  props.flow.elements.forEach(el => {
    ...
    const completedAt = el.completedAt ? new Date(el.completedAt).toISOString() : ''
    const expectedEndedAt = el.expectedEndedAt ? new Date(el.expectedEndedAt).toISOString() : ''
    csv += `"${el.id}","${el.name || ''}","${el.description || ''}","${el.type}","${el.status}","${owner}","${consultedUsers}","${completedAt}","${expectedEndedAt}","${predecessorIds}","${predecessorTypes}","${successorIds}","${successorTypes}"\n`
  })
  const blob = new Blob([csv], { type: 'text/csv' })
  const link = document.createElement('a')
  link.href = URL.createObjectURL(blob)
  link.download = (props.flow.name || 'flow-instance') + '.csv'
...
async function downloadPng() {
  ...
  try {
    ...
    const link = document.createElement('a')
    link.href = dataUrl
    link.download = (props.flow?.name || 'flow-instance') + '.png'
    document.body.appendChild(link)
    link.click()
    document.body.removeChild(link)
    shareDropdownOpen.value = false
...
const usedUserIds = computed(() => {
  if (!props.flow?.elements) return new Set<string>()

  const userIds = new Set<string>()

  props.flow.elements.forEach(element => {
    // Add owner IDs
    if (element.ownerId) {
...
const usedTeamIds = computed(() => {
  if (!props.flow?.elements || !teams.value.length) return new Set<string>()
...
const getFlowStatus = (): string => {
  if (!props.flow) return 'unknown'
  if (props.flow.completedAt) return 'completed'
  if (!props.flow.startedAt) return 'not-started'
  
  // Check if overdue
  if (props.flow.expectedEndDate && new Date(props.flow.expectedEndDate) < new Date()) {
    return 'overdue'
  }
...
const getFlowStatusLabel = (): string => {
...
const getFlowProgress = (): number => {
  if (!props.flow) return 0
  if (props.flow.completedAt) return 100
  if (!props.flow.startedAt) return 0
  
  // Only count action elements for progress
  const totalElements = props.flow.elements.filter(el => el.type === 'action').length
  
  if (totalElements === 0) return 0
  
  // Count completed action elements
  const completedElements = props.flow.elements.filter(el => 
...
// Calculate duration range (for flow instances, this is actual vs expected)
const durationRange = computed(() => {
  if (!props.flow) return { min: 0, max: 0 }
  // For flow instances, we can calculate actual time elapsed
  // TODO: Implement flow instance duration calculation
  return { min: 0, max: 0 }
})
...
const loadFlowIntoViewer = (flow: Flow) => {
...
// Reorganize layout using the same algorithm as loadFlowIntoViewer
const reorganizeLayout = () => {
  if (!props.flow) return
  loadFlowIntoViewer(props.flow)
}
...
// Initialize data when flow changes
watch(() => props.flow, (flow) => {
  if (flow) {
    loadFlowIntoViewer(flow)
  } else {
    nodes.value = []
    edges.value = []
  }
